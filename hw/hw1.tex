...
\begin{solution}
	...
	As an example, in the category \textbf{Grp}, $A \xrightarrow{f, f'} B \xrightarrow{g, g'} C$ are groups and group homomorphisms where $\exists b \in B$ such that $f(a) = bf'(a)b^{-1}$ for all $a \in A$ and similarly $\exists c \in C | g(b) = c g'(b) c^{-1} \forall b \in B$. Let's show that the equivalence preserves associativity with this setup: $(g \circ f)(a) = g(bf'(a)b^{-1}) = c g'(b f'(a) b^{-1})c^{-1} = c g'(b) g'(f'(a)) g'(b^{-1}) c^{-1} =  (cg'(b)) g'(f'(a)) (cg'(b))^{-1} \simeq (g' \circ f')(a)$. 
