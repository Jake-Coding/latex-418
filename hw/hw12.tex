...
\begin{solution}
	...
	Firstly, note that the identities $l, r, a$ are obviously natural, and moreover do what we asked them to by definition of the identity functor (for $l$ and $r$)  and associativity of functor composition (for $a$). 
...
\begin{solution}
	...
	I'll begin by claiming that $\mc{C}$ has the monoidal structure $(\mc{C}, \otimes', FI, a', l', r')$ where if $A, A' \in \mc{C}, A \otimes' A' = F( F^{-1}A \otimes F^{-1}A')$, and similarly on morphisms. We also define $a'_{X, Y, Z} = F(a_{F^{-1}X, F^{-1}X, F^{-1}Z})$, and we define $l'$ and $r'$ identically. 
...
\end{tikzcd}\]
	...
	This diagram, (naturality of $a$ followed by a monoid axiom), tells us that as we associate $(c \otimes c') \otimes c'' \mapsto c \otimes (c' \otimes c'')$, the relevant morphisms into $v$ associate to become $m \circ f \otimes (m \circ f' \otimes f'')$. Note that  
	
		\[(c, f) \otimes' ((c', f') \otimes' (c'', f'')) = (c, f) \otimes' (c' \otimes c'', m \circ f' \otimes f'') = (c \otimes (c' \otimes c''), m \circ f \otimes (m \circ f' \otimes f'')) \]
	
	Hence, performing our naively-defined tensor composition on the slice category is compatible with naively performing association on the object part of the slice category. We just take association in the slice category to be this naive method of association. Naturality of $a'$ follows by naturality of $a$ and the previous argument about how morphisms associate. 
...
%	\end{tikzcd}\]
		...
		We can also show that we satisfy the triangle axiom, but that is more symbol juggling, and ends up using all the equalities I've outlined above. Suffice it to say, we have a monoidal category! 
		
		A monoid in $\mc{V}/v$ is some object $(w, \omega: w \to v)$ with morphisms \[(w \otimes w, m \circ \omega \otimes \omega) \xrightarrow{\mu} (w, \omega) \text{ and } (I, i) \xrightarrow{\iota} (w, \omega)\]
