...
\begin{solution}
	...
	There is another interesting question, though, if we suppose we only have binary products...
	
	
	Suppose WLOG $A \times B = A$. 
	
	% https://q.uiver.app/#q=WzAsNCxbMSwwLCJBIl0sWzEsMSwiQSJdLFswLDIsIkEiXSxbMiwyLCJCIl0sWzAsMSwiZiIsMCx7InN0eWxlIjp7ImJvZHkiOnsibmFtZSI6ImRhc2hlZCJ9fX1dLFsxLDIsInBfMSJdLFsxLDMsInBfMiIsMl0sWzAsMiwiMV9BIiwyXSxbMCwzLCJwXzIiXV0=
	\[\begin{tikzcd}
		& A & \\
		& A \\
		A && B
		\arrow["f", dashed, from=1-2, to=2-2]
		\arrow["{1_A}"', from=1-2, to=3-1]
		\arrow["{p_2}", from=1-2, to=3-3]
		\arrow["{p_1}", from=2-2, to=3-1]
		\arrow["{p_2}"', from=2-2, to=3-3]
	\end{tikzcd}\]
	
	If $f \neq 1_A$, then $A$ cannot be initial or terminal. 
	
	So, for the sake of argument, suppose $f = 1_A$. Then $p_1 = 1_A$. This forces $p_2$ to be unique. 
	
	Let $f: B \to A$ be a morphism.
	
	% https://q.uiver.app/#q=WzAsNCxbMSwxLCJBIl0sWzAsMiwiQSJdLFsyLDIsIkIiXSxbMSwwLCJCIl0sWzAsMSwiMV9BIl0sWzAsMiwicF8yIiwyXSxbMywyLCIxX0IiXSxbMywwLCJmIiwwLHsic3R5bGUiOnsiYm9keSI6eyJuYW1lIjoiZGFzaGVkIn19fV0sWzMsMSwiZiIsMix7InN0eWxlIjp7ImJvZHkiOnsibmFtZSI6ImRhc2hlZCJ9fX1dXQ==
	\[\begin{tikzcd}
		& B & \\
		& A \\
		A && B
		\arrow["f", dashed, from=1-2, to=2-2]
		\arrow["f"', dashed, from=1-2, to=3-1]
		\arrow["{1_B}", from=1-2, to=3-3]
		\arrow["{1_A}", from=2-2, to=3-1]
		\arrow["{p_2}"', from=2-2, to=3-3]
	\end{tikzcd}\]
	
	This tells us that $f$ is unique and a right inverse to $p_2$ ($p_2 \circ f = 1_B$). 
	
	Could it be a left inverse please :(
