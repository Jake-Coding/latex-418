...
Given some $(s, y) \in \coprod_S f^{-1}(s)$ (thinking about the disjoint union as a tagged union), we take $\Phi_{Y, f}((s, y), \pi_f) = (y, f)$.
 ...
 Moreover, $\Phi$ is invertible: we can define $\Phi^{-1}_{Y, f}(y, f) = ((s, y), \pi_f)$ where $s$ is an element (choose one!) such that $y \in f^{-1}(s)$. Then, $\Phi \circ \Phi^{-1}$ is clearly the identity, and $\Phi^{-1} \circ \Phi$ is isomorphic to the identity (by choosing the "right" $s$). We can just redefine $\Phi$ to choose such $s$, and this gives us that the natural transformation $\Phi$ is in fact a natural isomorphism from $JK$ to the identity. Hence, $JK \simeq 1$. 
 
 Lastly, we show that $KJ \simeq 1$. 
...
Hence, $KJ \simeq 1$, and we are done! \qed
