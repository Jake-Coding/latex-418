...
\begin{solution}
	...
	Let $I$ be the identity map on $V$. It is clear that the identity map $1_{V^{**}}$ on $V^{**}$ is the map sending a linear functional $f : V^* \to \F$ to itself. In particular, $1_{V^{**}}(g)(f) = g \circ f$. However, $I^{**}(g)(f) = g \circ f$, so $I^{**} = 1_{V^{**}}$. 
...
The top diagram is the generic axiom. The second one has our constructions. The third one applies $F$ to the first one and introduces a number of commuting diagrams. The left trapezoid is naturality of $l'$ (notice $Fl = l'$). The upper triangle is by the equivalence. The right triangle commutes by our definition of $G_2$, and the bottom square is naturality of $\eps$. Because $F$ is an equivalence, it preserves diagrams in "reverse" (if $FD$ commutes, then $D$ commutes, as we could apply $G$ at the front again, then take $\eta^{-1}$ to get back to the identity). We can use the same sort of arguments---applying $F$ and using the same sorts of axioms from $F$ to show that the diagram commutes, then applying $G$ and taking $\eta^{-1}$ to get a commutative diagram that gives the axiom for $G$---and we're done! 
...
Then $F: \mc{A} \to \mc{V}$ is a strict monoidal functor, but $G: \mc{V} \to \mc{A}$ sending $*\mapsto 0$ has $G* \neq 1$, even though $G* \iso 1$. We know that $F, G$ form an equivalence of categories, and so we have found a counterexample!
