...
\begin{solution}
	...
	\[\begin{tikzcd}
		...
		& {1_A} & {\alpha_B^{-1}(1_B)} \\
		& {\alpha_A(1_A)} & {\alpha_A(1_A) \circ \alpha_B(1_B)=\alpha_B(\alpha_B^{-1}(1_B)) = 1_B} \\
		{H_B(A) = \mc{C}(A, B)} &&& {H_B(B) = \mc{C}(B, B)}
		\arrow["{H_A(h^\op) := - \circ h}", from=1-1, to=1-4]
		\arrow["{\alpha_A}"', from=1-1, to=4-1]
		...
		\arrow[maps to, from=2-3, to=3-3]
		\arrow[maps to, from=3-2, to=3-3]
		\arrow["{H_B(h^\op) := - \circ h}"', from=4-1, to=4-4]
	\end{tikzcd}\]
...
\begin{solution}
	...
	Let $f, g: A \to B$ in $\mc{C}(A, B)$. We write $H_f: H_A \natt H_B$, where components $(H_f)_{A}: H_A(A) \to H_B(A)$ for $A$ are given by $- \circ f$, and similarly for $g$. 
	...
	Suppose that $H_f = H_g$. They must be equal at all components. In particular, for any $A$, $(H_f)_A (h) = h \circ f  = (H_g)_A h = h \circ g$. We choose $h = 1_A$. Then, $f = g$.  \qed
